...
| Any DM-Ops tables | `docket`, `timesheet`, `employee`, `crew`, `asset`, `tender`, `purchase_order`, `invoice` — not in this schema. The nightly booked-collection sync is RETIRED (23/08, ADR 0015): cron unscheduled, `nightly-sync-to-dm-ops` EF dormant — never "fix" or re-schedule it without a payload redesigned against DM-Ops' real schema |
...
6. **Never write to DM-Ops tables from Verco application code** — only `nightly-sync-to-dm-ops` Edge Function touches DM-Ops (dormant since ADR 0015; the red line still stands)
...
### Admin + white-label UI — admin list surfaces compose `PageHeader`/`FilterBar`/`SearchInput`/`FilterSelect`/`Th`/`Pagination` (`components/admin/`) + `<StatusBadge entity status>` (`components/status-badge.tsx`) — never re-type header/filter/pill/pagination markup inline. Status colours come ONLY from the semantic pairs in `lib/ui/status-styles.ts` (backed by `--color-status-{success,warn,error,info}` + `-bg` in globals.css); the type scale is token-only — no arbitrary `text-[Npx]` (12px=`text-xs`, 14px=`text-sm`, uppercase section labels=`text-caption`, page titles=`text-xl` via PageHeader); keyboard focus rings come from the `.admin-surface :focus-visible` base rule (any `outline-none` must pair an explicit affordance); admin `<table>`s carry `tabular-nums`. White-label (public/field surfaces; admin exempt) uses CSS vars not hex — `--brand`/`--brand-accent`/`--brand-foreground` + `-light`/`-hover`/`-dark`; `text-white` silently fails under Tailwind v4 + Turbopack, use `--brand-foreground` (defaults `#FFFFFF`) with an inline `style={{ color }}` fallback (memory `tailwind-v4-turbopack-gotcha.md`). FAQ markdown: `components/faq-answer.tsx` is directive-free (RSC/client dual-render); NO `rehype-raw` EVER — admin-authored multi-tenant content on public pages, raw HTML stays inert; `tel:` links via `urlTransform`. Full developer reference: `docs/admin-design-system.md`. Per-tenant copy/colour asks = nullable `client` cols (`office_hours`, `hero_colour` — hero background ONLY, never a primary/accent swap: that restyles the whole resident surface, 29/07 incident) or jsonb maps (`service_descriptions`) with code fallbacks; NULL = legacy rendering.

### Public-SELECT RLS (`USING(true)`) doesn't tenant-scope — filter in app. `eligible_properties`, `collection_area`, `collection_date`, `client` etc. are cross-tenant readable for the unauthenticated `/book` flow. **Exception — SUB-client narrowing IS in the DB** (ADR 0019, migration `20260901010000`): every SELECT policy on `collection_area`/`eligible_properties`/`collection_date`/`allocation_rules`/`service_rules` carries `(SELECT current_user_sub_client_id()) IS NULL OR <area in own sub-client>` — caller-based, defaults open for anon/residents/unscoped staff, so /book is untouched; a new SELECT policy on these tables MUST repeat the predicate (policies OR — one bare policy re-opens the leak). Server pages must read `x-client-id` from `headers()`, pass `clientId` to client components, and queries must join via embedded `!inner` FK + `.eq('<fk>.client_id', clientId)`. See `book/page.tsx` + `book/address-form.tsx`. Same trap for **which client an admin may act as**: validate the switcher cookie / `x-client-id` against `accessible_client_ids()` (SETOF; `id IN (SELECT accessible_client_ids())` or fetch-then-`.in`), NEVER re-query the public-SELECT `client` table by `is_active` alone — that validates any active client id → cross-tenant switcher leak (P1, fixed in `lib/admin/current-client.ts`, #275). Memory `admin-switcher-public-select-client-scope.md`. Admin PICKER options are the same trap — narrow via `fetchAccessibleClientOptions` (`lib/admin/accessible-clients.ts`), never a bare active-clients read (#456, third occurrence of this class).

### Edge Functions + notifications — EFs that access PII accept dual auth (§20 Red Line #3): a service-role bearer (EF→EF) OR a user JWT whose `current_user_role()` is in a permitted set; server actions MUST NOT use service role. Notification sends go through `templates/template-helpers.ts` + `invokeSendNotification` (`src/lib/notifications/invoke.ts`); idempotency keys on `(booking_id, type, channel)` NOT `(booking_id, type)` so email + SMS succeed independently (new channels follow suit). `_shared/`↔`src/lib` mirror pairs are kept in sync by `scripts/sync-mirrors.sh` (`_shared/` is source of truth; a NEW pair MUST be registered there or it drifts silently past CI). DB-column / EF-envelope changes follow migration → EF-with-back-compat-shim → Coolify → strip-shim; EF responses emit documented fields on every path (success / no-op / error) — defaults belong in the EF, not the parser. **Field-surface contract changes:** crew phones run the PWA for days — `useRefreshOnFocus` refreshes RSC data, never the JS bundle — so a server action gaining a required param MUST accept its absence for one release (fail-closed on the new param = field outage: 03/08 mattress gate stranded every VV bulk closeout, ADR 0011); the run-sheet focus hook hard-reloads once on `/api/health` SHA mismatch; Next itself hard-navigates a stale bundle on its next soft navigation / `router.refresh()` (build-id check on every RSC response), so the ONLY gap is a server action called first — action ids are salted per build (`NEXT_SERVER_ACTIONS_ENCRYPTION_KEY` unpinned → EVERY deploy 404s every stale call as `UnrecognizedActionError`), so every action `catch` shows `staleActionMessage(err) ?? <its own message>` and `instrumentation-client.ts` + `global-error.tsx` net bare awaits / form actions (`lib/bundle/stale-action.ts`, one reload per 30s, ADR 0023) — never report it as "No connection"; field error banners render ABOVE the CTA (below-the-fold rejection reads as a dead button). **Observability + scanning:** Sentry is wired for the app + EFs (env-gated on `NEXT_PUBLIC_SENTRY_DSN` / EF `SENTRY_DSN`, PII-scrubbed via `src/lib/sentry/scrub.ts` + `_shared/sentry.ts`'s `withSentry`; tracesSampleRate 1.0; EF SDK = `@sentry/deno` via esm.sh; Sentry project is EU-region) — NEVER enable Session Replay / `includeLocalVariables` / `enableLogs` (council PII must not leave AU). Secret+SAST scanning gates every push/PR: gitleaks + semgrep + a local pre-commit hook in `scripts/git-hooks` (NOT the Python `pre-commit` framework — it refuses to install under the repo's `core.hooksPath`); public anon keys allowlisted in `.gitleaks.toml` (semgrep = SAST only, ERROR-gated). Memory: `edge-function-patterns.md`, `shared-lib-patterns.md`, `feedback-multi-channel-idempotency.md`, `verco-sentry-observability.md`, `verco-secret-sast-scanning.md`, `verco-infra-hardening.md`. Phone handling has ONE brain: `_shared/phone.ts` → `src/lib/phone.ts` mirror (mobiles→E.164, landlines/1300 stored formatting-stripped, `isSmsCapable` drives no-SMS hints; resident + strata + EF schemas AND ops/import scripts all go through it — never write a phone cell verbatim: the 23/08 VIN import did and 991/1,001 contacts were un-SMS-able, #543). Service-role recognition in dual-auth EFs = `isServiceRoleBearer` (`_shared/service-role-auth.ts`: exact env-key match, else a CLAIMED service role is proven against the gateway) — never exact-string-match the env key (key-format rotation 401'd CLI callers, #480); user-JWT callers of EFs that run service-role queries must be pinned to `accessible_client_ids()` (#530). A money reconcile (Stripe session → booking) must never sit inside a `catch` that falls through to "mark expired + mint a new session" — that path re-charged paid residents for 7 weeks (#496); reconcile failures are 500s. Manual EF secrets (anything non-`SUPABASE_*`) are NOT platform-injected — never feed `Deno.env.get(x)!` straight into a constructor before the try/handler: a missing secret throws pre-log (`supabaseUrl is required` hid every nightly DM-Ops run for 5 months, PR #535); read the env var, guard with an error NAMING the missing secret, and record a failure row first.

### Schema + PostgREST query gotchas — `booking_survey.booking_id` is NULLABLE for `source='airtable'` (imported legacy surveys, ADR 0016): council lives on `booking_survey.collection_area_id` (trigger-filled from the booking for Verco-native rows) — staff RLS, the CSAT series and every admin reader key off THAT, never `booking!inner`. `contacts.full_name` is `GENERATED ALWAYS AS STORED` (read-only; forms capture `first_name` + `last_name` as separate required fields, select `full_name` for display only). Generated STORED cols over NOT NULL inputs need an explicit `ALTER COLUMN … SET NOT NULL` or regen'd TS is `string | null` (memory `schema-conventions.md`). `.or()` + embedded-select: (1) multi-FK embeds silently return empty inner for authed users — use `related!fk_name(col)` or split-query+stitch; (2) `.or()` can't filter parents by a nullable LEFT-joined table's columns — pre-fetch ids + `.in(...)`; (3) any `.or()` value with a comma / PostgREST-reserved char MUST be double-quoted or `PGRST100` 400s swallow into `data:null` (address/search strings always hit this). Helpers `buildEligibleOrFilter` + `buildSearchOrFilter` (`lib/search/or-filter.ts`); canonical `admin/bookings/bookings-list-client.tsx`. `useState(searchParams.get(...))` doesn't re-init on same-path soft nav — sync with `useEffect(() => setX(searchParams.get('x') ?? ''), [searchParams])`.

### Migrations + deploy — never Supabase MCP `apply_migration` on prod (stamps `version=now()`, blocks the next `db push`); always `migration new` → file → CI `db push`. NEVER reuse a 14-digit version prefix already applied to prod (dup → `42701` aborts the whole batch incl. EF + Coolify steps); scan for duplicate prefixes in pre-release review. Types Freshness CI gens from prod, so new-RPC + new-consumer in one PR fails CI — split PR-A (migration) → release → PR-B (consumer + regen'd types); regen `types.ts` with the LOCKFILE-pinned `supabase` (node_modules / `npx supabase@<pinned>`), NOT the global CLI — version skew adds/drops schema blocks (e.g. `graphql_public`) failing the byte-exact Types-Freshness diff, so diff the regen vs the branch base to confirm the delta is ONLY your object. Data-seed migrations keyed on later-seeded rows (e.g. KWN in `seed.sql`) must no-op on a fresh `db reset` — assert an invariant, never a hardcoded count. `NEXT_PUBLIC_*` is baked at build time via Docker build-args (`deploy.yml`) — Coolify runtime env is a no-op; new vars go in `.env.example` + GitHub secrets + `deploy.yml` build-arg + Dockerfile `ENV`. Auth email + the `[auth]` block live in `supabase/templates/*.html` + `config.toml`, applied via `supabase config push` — it syncs the ENTIRE `[auth]` block (never `--yes` unreviewed, no undo); GoTrue uses Go `html/template` (no sprig), parse errors silently fall back to defaults — test with a fresh OTP. Verify prod migrations via `supabase db query --linked` + RAISE-rollback. Memory: `mcp-apply-migration-version-sync.md`, `ghost-release-pattern.md`, `seed-migration-reset-safety.md`, `feedback-supabase-config-push-stdin.md`, `deploy-verification-url-gotcha.md`, `types-regen-cli-version-skew.md`. **Council cutover from Airtable (VV):** bookings + surveys come in via CSV exports of the Airtable MASTERS (`scripts/import-vv-bookings-csv.ts`, `scripts/import-vv-surveys-csv.ts`, dry-run default, idempotent) — import from FY START not just future rows, or `get_property_fy_usage` shows a full allocation and paid extras go free (memory `vv-council-csv-booking-import.md`). Ops/import scripts: `rg` the flags you pass in the CHECKED-OUT tree first (`parseFlags` ignores unknown flags; a stale worktree ran the full MUD import against prod, 29/07) and always `--only`/`--forms-only` (re-runs clobber geocodes). **Ghost migrations:** a `column … does not exist` log error on a table an EF WRITES is a P1 until proven otherwise — `schema_migrations` can record a version whose DDL never ran (`receipt_url`, 02/04→22/08: every paid-booking confirmation path threw for 7 weeks, #496); grep every writer, repair with a NEW version + `IF NOT EXISTS`. **Pre-release:** run `grep -rn --exclude-dir=__tests__ SUPABASE_SERVICE_ROLE_KEY src/` — `deploy.yml`'s Red Line #3 gate is a plain grep that fails the image build on COMMENTS too, incl. mirrored `_shared` helpers, AFTER the release PR merged (#520); and a migration hotfix to `main` flips Types Freshness red for the NEXT deploy — regen types in the same hotfix chain (#514 unblocked #513).
...
### Capacity + booking gates — capacity-checking RPCs MUST branch on `collection_area.capacity_pool_id`: pooled areas (VV) keep counters on `collection_date_pool` (checking/locking `collection_date` neither enforces nor serialises) — mirror `create_booking_with_capacity_check`'s pooled branch, and merge pool counts in any UI showing per-date counters (else phantom capacity). Staged go-live (`collection_area.is_active`, WS-A): only active areas are bookable, enforced at 4 layers — client fail-OPEN (`!== false`), create-booking EF + `createMudBooking` + the RPC fail-CLOSED, and `booking_resident_insert` RLS `WITH CHECK collection_area_is_active(...)` (the helper COALESCEs false). It's data-driven: `is_active` defaults true, so holding a council back is an admin toggle, never a migration (create-then-toggle-off for a not-yet-live council). `is_active` gates RESIDENT booking only — admin/staff surfaces (area pickers, filters, imports) must NOT filter on it: staff load a council's properties before go-live (#604 hid 6 VV councils from Add property). Cancellation cutoff: NEVER `Date#setHours()` (runtime-TZ-dependent, wrong on the UTC prod box) — use `cancellationCutoff`/`isPastCancellationCutoff` (`src/lib/booking/cancellation-cutoff.ts`; 07:30 UTC via `Date.UTC`, matches the DB trigger `enforce_cancellation_cutoff`). Field crew model: a `collection_stop` = booking × `service.waste_stream`, generated only by the push-to-OptimoRoute EF at T-3; booking status derives from stops via `rollup_booking_status_from_stops` (never set directly when stops exist). Job type for crews (MUD/ID): `booking.type` read at render/push time via `bookingTypeTag` (`_shared/stops.ts` mirror) → `BookingTypeBadge` (crew card, closeout, admin run sheet — never `print:hidden`) + the OR notes' leading `Type:` line — never denormalised onto the stop; `unit_count` 0/1 = unknown, never shown (migration `20260522143000`). Mattress counts (#487): `client.mattress_closeout_stream` names the pass that prompts crews (NULL = never; VV = general), `service.is_mattress` keys the KWN booked series (never the display name); `collection_stop.mattress_count` is written ATOMICALLY with the terminal status (the only moment the field UPDATE WITH CHECK admits it — no separate save path, no RLS change), non-logging tenants' submitted counts are DISCARDED server-side; NULL = never logged (accepted + alerted, never blocks a closeout — ADR 0011), 0 = crew logged zero; reports read `get_mattress_daily` (day-granular, both sources). ADR 0022, memory `mattress-counts-feature-state.md`. `collection_stop` is an IMMUTABLE as-dispatched record (identity cols incl. `collection_date_id` are trigger-locked to service-role): a #378 admin date-override edits `booking_item.collection_date_id` (corrected intent) but NOT the stop (dispatched history), so the contractual on-time KPI (`on-time.ts`, `get_on_time_monthly`) MUST key off the stop's date — repointing it to the booking date would launder a wrong-day miss into an on-time success. stop=dispatched, booking=corrected; never "fix" the divergence (#390.2 D2). A date move DOES immediately cancel (never repoint) the booking's still-`Pending` stops for the vacated (date, stream) — `sync_stops_on_booking_item_date` trigger, ADR 0018 — so the hourly OR sweep deletes the phantom order within the hour instead of at the next 03:10 push run (KWN-4-X96WUS sat in OR for 19h and was route-planned for a day it was no longer booked); terminal stops are untouched and the push EF revives the row when the new date locks. **Crews get their routes at 8pm the night before — that is the deadline every OR-facing change must beat** (ADR 0018), so the push EF is HOURLY (`10 * * * *`, migration `20260827020000`), not the old daily 03:10 AWST which landed 7h AFTER dispatch; a move onto an already-locked date used to miss the crew entirely. Safe hourly because pass 2 pushes only `pushed_at IS NULL` and `payloadDiffers` resets it only on a real change (`servicesSummariesEqual` guards the 12/07 jsonb key-order refresh-storm). Push-EF pass-1 diffs via pure `planStopChanges()` (`_shared/stops.ts`) over stops fetched by locked DATE **and** by desired `booking_id` — a date-windowed fetch missed a booking's Cancelled stop once its old date rolled past, and `upsert(ignoreDuplicates)` against the non-partial `UNIQUE(booking_id, stream)` silently no-op'd: 17 bookings never dispatched (#486). Any `ignoreDuplicates` upsert logs attempted−inserted (`stops_insert_skipped`). **Date moves:** client-tier moves are capacity-gated (`capacityBlocksMove`, picker + action + trigger), contractor keeps the closed/past/full override (ADR 0014); the `enforce_booking_item_staff_write` trigger also pins price/identity cols and the booking's area for EVERY user role and mirrors `canEditCollectionDetails` for client-tier (ADR 0012 — trigger, not RLS: per-row lookups in `USING` are the initplan-timeout class). **Properties** are corrected in place (`updateEligiblePropertyAddress` auto re-geocodes via `geocode-properties` `property_ids`; area move contractor-only + blocked while non-terminal bookings exist) — never "mark ineligible + recreate", which hands the new row a fresh FY allocation (ADR 0013). **FY allocation-usage reads in the /book flow MUST go through the `get_property_fy_usage` SECURITY DEFINER RPC (PII-free per-service+per-category counts + a `('swap', <rule_id>, 1)` row when the property has an applied allocation swap — the FY-long forfeiture EVERY pricing surface must honour, design §2; `p_exclude_booking_id` also suppresses the swap row for the swap-origin booking so edits stay governed by the explicit `swap` flag), NEVER a direct `booking`/`booking_item`/`allocation_swap` read — the services step runs pre-OTP, so an anonymous resident's RLS-scoped read returns zero (shows full allocation "X of X" after booking + prices used units free; for `allocation_swap` it re-offered an already-used swap). Feeds services-form, confirm-form, address-panel, and `calculatePrice` (which derives the existing-swap conversion itself; explicit param wins) in the EF.** The address panel's Booking history list is the same trap — it reads `get_property_fy_booking_history` (PII-free ref/status/created_at, newest 5), never `booking` directly (#582: anon saw "No bookings yet" under a tile showing real usage). Memory: `field-stops-optimoroute-architecture.md`, `booking-write-paths-and-gating.md`, `category-code-equals-capacity-bucket.md`, `pre-otp-allocation-usage-rpc.md`.
