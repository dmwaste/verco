 ...
 *
 * ⚠️ Deno mirror — the source of truth is the Node copy at
 * `src/lib/notifications/health.ts` (typechecked + unit-tested under Vitest;
 * `supabase/functions` is excluded from tsconfig/vitest). This file is a
 * verbatim copy imported by the Edge Function. Keep the two in sync manually
 * (same pattern as the dispatcher's `dispatch.ts`).
 */
