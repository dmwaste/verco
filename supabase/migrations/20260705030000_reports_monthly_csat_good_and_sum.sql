-- ============================================================================
-- get_reports_monthly — CSAT: emit BOTH "% rated 4+" and average series (05/07)
-- ============================================================================
-- Correction to 20260705020000. Two surfaces now read the CSAT sparklines and
-- they want DIFFERENT metrics off the same fold:
--
--   * /admin/reports  → AVERAGE rating (sum/n)          → needs _sum + _n
--   * /admin/surveys  → WMRC KPI "% rated 4+" (good/n)  → needs _good + _n
--
-- 20260705020000 dropped _good in favour of _sum; the Surveys card is meant to
-- stay on the WMRC KPI, so this re-adds _good and keeps _sum. Three series per
-- key now:
--     csat_{booking,service,overall}_n     count of valid 1..5 ratings
--     csat_{booking,service,overall}_good  count rated 4..5  (WMRC KPI)
--     csat_{booking,service,overall}_sum   SUM of rating values (average)
--
-- CREATE OR REPLACE of the released body — every other series arm, both gates,
-- the 8A contractor filter and the search_path pin are reproduced verbatim; ONLY
-- the CSAT UNION arm changed (kind 1/2/3 → _n/_good/_sum, r.kind in GROUP BY
-- because the value aggregate branches on it). ACLs preserved by OR REPLACE;
-- REVOKE/GRANT re-stated. No table locks → no lock_timeout.
-- ============================================================================
