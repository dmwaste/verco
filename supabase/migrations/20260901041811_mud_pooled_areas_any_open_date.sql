-- #558 option 2: pooled areas accept MUD bookings on ANY open collection
-- date. The per-date for_mud flag was a carryover from the dedicated-MUD-day
-- model (unpooled councils like KWN, where crews run specific MUD days) and
-- defaulted false at date generation — leaving every pooled (VV) council's
-- MUDs unbookable by default (BR-0036).
--
-- 1. Re-declare create_mud_booking_with_capacity_check with the relaxed date
--    gate: (v_pool_id IS NOT NULL OR cd.for_mud). Everything else is verbatim
--    from 20260819060000 (the latest declaration). The inline SET search_path
--    pin is re-declared with the function; grants/revokes survive OR REPLACE
--    unchanged. App-side mirrors: isMudCollectionDate (src/lib/mud/capacity.ts),
--    the MUD book page date query, and createMudBooking's validation.
--
-- 2. Revert the 01/09 interim data toggle: for_mud=true was bulk-set on 299
--    upcoming VV dates to unblock MUD booking while this fix was built. The
--    flag is inert on pooled areas under the new rule, so reset it there to
--    keep the admin dates UI honest (the MUD pill/toggle stays meaningful
--    for unpooled councils only). Reset-safe: no-ops when no rows match.
...
BEGIN
  ...
  WHERE cd.id = p_collection_date_id
    ...
    -- Pooled areas accept MUDs on any open date (#558 option 2); unpooled
    -- councils keep the curated for_mud dedicated-MUD-day model.
    AND (v_pool_id IS NOT NULL OR cd.for_mud)
    AND cd.is_open
...
-- ── 2. Reset the now-inert flag on pooled areas (incl. the 01/09 VV toggle) ──
UPDATE collection_date cd
SET for_mud = false
FROM collection_area ca
WHERE ca.id = cd.collection_area_id
  AND ca.capacity_pool_id IS NOT NULL
  AND cd.for_mud;
