 ...
 * human-readable alert. The Edge Function
 * (`supabase/functions/notification-health-check/index.ts`) wires the real
 * `notification_log` queries + alert webhook into this contract.
 *
 * Mirror: this `_shared/` file is the source of truth. Its Node twin at
 * `src/lib/notifications/health.ts` is generated by `scripts/sync-mirrors.sh`
 * (which only strips Deno's `.ts` import extensions) so it can be typechecked
 * + unit-tested under Vitest — `supabase/functions` is excluded from
 * tsconfig/vitest. Edit here, then run the script; the mirror CI `--check`
