...
describe('checkRebookDate', () => {
  it('allows a normal open date', () => {
    expect(checkRebookDate(gate(), ['bulk'])).toEqual({
      bookable: true,
      reason: null,
  ...
  it('allows a date closed only by the T-3 lock — the WMRC ad-hoc ask', () => {
    expect(checkRebookDate(lockedGate, ['bulk'])).toEqual({
      bookable: true,
      reason: null,
  ...
  it('refuses a full bucket even inside the lock window', () => {
    ...
    expect(checkRebookDate(full, ['bulk'])).toEqual({
      bookable: false,
      reason: 'full',
      insideLockWindow: true,
    })
  ...
  it('refuses a public holiday or admin-closed date', () => {
    expect(checkRebookDate(gate({ is_open: false }), ['bulk']).bookable).toBe(false)
    // Even when the lock also applies.
    expect(
      checkRebookDate({ ...lockedGate, is_open: false }, ['bulk']).bookable,
    ).toBe(false)
  })
  ...
  it('refuses a bucket an admin closed when the lock is not the cause', () => {
    ...
    expect(checkRebookDate(adminClosedBucket, ['bulk'])).toEqual({
      bookable: false,
      reason: 'closed',
      insideLockWindow: false,
    })
  ...
  it('checks every bucket the booking needs, not just the first', () => {
    ...
    expect(checkRebookDate(ancFull, ['bulk']).bookable).toBe(true)
    expect(checkRebookDate(ancFull, ['bulk', 'anc']).bookable).toBe(false)
    expect(checkRebookDate(ancFull, ['bulk', 'anc']).reason).toBe('full')
  })

  it('refuses an unknown or unconfigured bucket rather than waving it through', () => {
    expect(checkRebookDate(gate(), ['nonsense']).bookable).toBe(false)
  })

  it('exposes a plain boolean for callers that only need yes/no', () => {
    expect(isRebookDateBookable(lockedGate, ['bulk'])).toBe(true)
    expect(isRebookDateBookable(gate({ is_open: false }), ['bulk'])).toBe(false)
  })
})
...
describe('bucketsFromRow', () => {
  ...
  it('reads a pool member\'s inert own-row counters as zero capacity', () => {
    ...
    expect(checkRebookDate(g, ['bulk'])).toEqual({
      bookable: false,
      reason: 'full',
      insideLockWindow: false,
    })
