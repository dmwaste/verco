...
| You **can** | You **cannot** |
|---|---|
| Look up any address in the City of Kwinana's collection areas | See any resident's name, email, or phone number |
...
| Track the IDs you've raised and their collection status | See bookings or piles outside the City of Kwinana |
...
> **Your welcome email.** When D&M sets up your access you'll receive a "Welcome to Verco" email with a **Log In** button. It takes you to the same `field.verco.au` sign-in screen described above.
...
- A green pill listing **your area codes** — the patch you're scoped to. City of Kwinana rangers see the four Kwinana collection areas: `KWN-1 · KWN-2 · KWN-3 · KWN-4`.
...
<!-- SCREENSHOT: K03-frame — the ranger app frame with the KWN-1…KWN-4 area pill and the Lookup / New ID / My IDs tab bar -->

### 1.5 Your patch — how your areas are set

You only ever see addresses, piles, and history for the **collection areas assigned to your login**. A D&M administrator sets this up — you can't widen it yourself, and you don't need to pick an area anywhere. A City of Kwinana ranger is scoped to Kwinana Areas 1–4 and simply won't see any other council's data.
...
1. Tap the **Lookup** tab. The heading reads **Address Lookup**, with *"Check whether a pile belongs to a booking — City of Kwinana"* underneath.
...
<!-- SCREENSHOT: K04-lookup — Address Lookup with a Kwinana street typed in and matching properties listed -->

**Two things you might see instead of results:**

- **"No matches — Address not eligible for verge collection — likely a candidate for an ID."** The address isn't on the City of Kwinana eligibility list at all. A pile here is very likely illegal dumping — go straight to raising an ID (§3).
...
<!-- SCREENSHOT: K05-verdict-green — "Likely a legitimate booking" banner on a Kwinana property -->
<!-- SCREENSHOT: K05-verdict-amber — "Booking exists, but placed out too early" banner -->
<!-- SCREENSHOT: K05-verdict-red — "No upcoming booking" banner -->
...
City of Kwinana residents may put their pile on the verge from **48 hours before** their collection day (that's the window the green/amber banners pivot on). Concretely: a pile out **within** that 48-hour window with a booking is **legitimate** (green); a pile out with a booking but **before** the window has opened is **too early** (amber); a pile with **no** booking is a candidate for an ID (red).
...
> **Kwinana's window is 48 hours, not 72.** If you've seen the Verge Valet version of this guide, note the difference — the app applies the City of Kwinana's own setting automatically, so the banner is always right for the address you're looking at.
...
| <span class="tl tl-a"></span> Booking exists — placed out too early | Booked, but early. City early-placement process, not an ID. |
...
<!-- SCREENSHOT: K06-id-form-top — the New ID form opened from a lookup, location pinned from the property, address editable -->
...
> **Keep the real street address in the address field.** If you replace it entirely with a landmark ("front gate, works depot"), the crew still gets your GPS pin — but the office loses the street address on the record. Best practice: leave the auto-filled address and add the landmark to **Additional Notes** (§3.6) instead.
...
<!-- SCREENSHOT: K06-id-form-bottom — Photos "Add photo" tile, the collection-date grid with ID spots per date, notes field, Submit button -->
...
Pick when the crew should collect. Each date tile shows the day and how many **ID spots** are left on that date — the City of Kwinana sets a small daily cap for illegal-dumping collections, separate from resident bookings. Tap a date to select it — it shows a **Selected ✓**. If a date is full it won't have spots available; pick another.

> **Book the earliest date with spots.** Choosing the earliest available date gets the pile cleared fastest. If the City later needs it moved, contact D&M with the booking reference — rangers can't change the date themselves.
...
<!-- SCREENSHOT: K09-id-confirm — green tick, "ID Collection Logged", summary card with the KWN-… booking reference -->
...
**Your booking reference** looks like your area code followed by six characters — e.g. `KWN-2-9ET76E`. Jot it down or trust **My IDs** to keep it (§4).
...
<!-- SCREENSHOT: K10-my-ids — the My IDs tab with at least one raised Kwinana ID card -->
...
| 2 | Green **"Likely a legitimate booking"** | Note the date, move on | Nothing to log — it's booked and within the 48-hour window |
| 3 | Amber **"placed out too early"** | Apply the City's early-placement process | Not an ID — it's a genuine booking, just early |
...
| 6 | Complete an ID with a photo and date | Submit | **ID Collection Logged**, a `KWN-…` reference, and it appears in **My IDs** as **Confirmed** |
...
No. There's a real booking; the resident was just early. Handle it through the City's early-placement process, not as an ID.
...
No. If a resident wants to book a collection, direct them to the VERCO Kwinana booking portal at `kwn.verco.au` or to the City's customer service. The ranger app is lookup-and-report only.
...
2. **Note the reference** if there is one (e.g. `KWN-2-9ET76E`).
3. **Note the address** and the **time** (Perth time) it happened.
4. **Say what you expected vs. what you saw** — a sentence each is plenty.
5. Send it to **D&M Waste Management**: email **`bookings@verco.au`** or phone **08 6336 7223** (Mon–Fri 8am–5pm AWST).

For anything urgent (the app is down, or you can't sign in at all), contact D&M directly on the number above, or via the City's usual contract contact.
...
- **1.0 — 2026-09-02**: Initial City of Kwinana release, derived from the Verge Valet Ranger Field App Guide v1.2 and re-verified against the live ranger surfaces (`app/(field)/field/lookup`, `.../illegal-dumping/new`, `.../my-ids`). Kwinana-specific values: **48-hour** place-out window, area codes **KWN-1…KWN-4**, `KWN-{area}-{code}` booking references, VERCO Kwinana portal at `kwn.verco.au`, D&M contact details. Waste-type list reflects the current app labels (**Bulk Waste** rather than "General / Mixed"). Added the address-field best practice (§3.2) and the earliest-date guidance (§3.5). Sign-in screenshots are shared with the Verge Valet guide (the sign-in screens are identical); Kwinana-specific screens (app frame, lookup, verdicts, ID form, confirmation, My IDs) are to be captured once the first City of Kwinana ranger login exists — see `docs/screenshots/kwn-ranger/KWN-RANGER-CAPTURE-CHECKLIST.md`.
