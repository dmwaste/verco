// scripts/import-kwn-mud-properties.ts
/**
 * Kwinana MUD List → Verco migration.
 *
 * Imports 73 MUD strata complexes from the KWN Airtable base
 * (apppzIjIc05ghcixH / tblsF6QreqldGGGuR) into eligible_properties,
 * contacts, and mud-auth-forms storage under the KWN client.
 *
 * Idempotent: external_source='airtable-mud-kwn', external_id=<Airtable record ID>.
 *
 * Usage:
 *   pnpm tsx scripts/import-kwn-mud-properties.ts            # full import
 *   pnpm tsx scripts/import-kwn-mud-properties.ts --dry-run
 *   pnpm tsx scripts/import-kwn-mud-properties.ts --skip-forms
 *   pnpm tsx scripts/import-kwn-mud-properties.ts --limit=10
 */
import { createClient } from '@supabase/supabase-js'
import { randomUUID } from 'node:crypto'
import { writeFileSync } from 'node:fs'
import { fetchAllKwnMudRecords } from './lib/airtable-mud'
import { loadAreaMapForClient, resolveAreaId } from './lib/area-map'
...
const EXTERNAL_SOURCE = 'airtable-mud-kwn'
const STORAGE_BUCKET = 'mud-auth-forms'

// ─── Helpers (same as import-mud-properties.ts) ───────────────────────────────
...
async function main() {
  ...
  console.log(`Loaded ${areaMap.size} Verco collection_areas for kwn.`)

  // ── Fetch KWN MUD records ──
  console.log('\nFetching KWN MUD List from Airtable…')
  let records = await fetchAllKwnMudRecords(airtableToken)
  ...
  for (const rec of records) {
    ...
    if (rec.frequencyMonths === 2) {
      report.cadenceApproximate.push({
        id: rec.id, address: rec.address,
        frequencyMonths: rec.frequencyMonths, mappedTo: cadence,
      })
    }
    ...
    // Dedup mud_code within the same area
  ...
  const reportPath = `import-kwn-mud-report-${timestamp()}.json`
